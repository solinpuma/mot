...
carga = slider.value();
  ...
  t4 = 2*pow(vel_rpm,2)/pow(10,5) - 0.0072*vel_rpm + 22.742;
  t3 = 6*pow(vel_rpm,2)/pow(10,5) - 0.1227*vel_rpm + 106.16;
  ...
  FC=60
   torque = -0.0006*pow(vel_rpm,2) +1.68*vel_rpm - 996;
  dp =  0.2*vel_rpm -36.833;
  p2 = p1-(dp/1000);
  p3 = (-1*pow(vel_rpm,2)/pow(10,6) + 0.0043*vel_rpm - 3.2608)*100;
  p4 = (-2*pow(vel_rpm,2)/pow(10,7) + 0.001*vel_rpm - 0.7)*100;
  p5 = (0.0037*vel_rpm - 2.8975)*100;
  p6 = (1*pow(vel_rpm,2)/pow(10,8) - 2*vel_rpm/pow(10,5) +0.0079)*100;
  t2 = -7*pow(vel_rpm,2)/pow(10,7) + 0.001*vel_rpm + 23.08;
  t4 = 8*pow(vel_rpm,2)/pow(10,6) + 0.0102*vel_rpm + 14.21;
  t3 = 5*pow(vel_rpm,2)/pow(10,5) - 0.0973*vel_rpm + 88.17;
  t5 =  0.1705*vel_rpm +57.595;
  t6 =  0.1729*vel_rpm +14.573;
  flow_ref = + 0.0048*vel_rpm - 0.4833;
  t_out = -1*pow(vel_rpm,2)/pow(10,5) + 0.0459*vel_rpm + 28.5;
  t_in = 0.0017*vel_rpm + 60;
  flow_d = 0.0057*vel_rpm - 1.7133;
 
   }else{
     FC=20
   torque = -0.0002*pow(vel_rpm,2) +0.56*vel_rpm - 332;
  dp = -0.0003*pow(vel_rpm,2) + 0.8725*vel_rpm -400;
  p2 = p1-(dp/1000);
  p3 = (3*pow(vel_rpm,2)/pow(10,7) - 0.0005*vel_rpm +0.266)*100;
  p4 = (3*pow(vel_rpm,2)/pow(10,21) + 0.0002*vel_rpm - 0.2)*100;
  p5 = (0.003*vel_rpm - 2.2983)*100;
  p6 = (7*pow(vel_rpm,2)/pow(10,9) - 1*vel_rpm/pow(10,5) +0.0028)*100;
  t2 = -5*pow(vel_rpm,2)/pow(10,6) + 0.0104*vel_rpm + 17.31;
  t3 = 5*pow(vel_rpm,2)/pow(10,5) - 0.0938*vel_rpm + 84.12;
  t4 =  0.0519*vel_rpm -23.548;
  t5 =  0.1547*vel_rpm +3.3083;
  t6 =  0.2*vel_rpm -120.29;
  flow_ref = 3*pow(vel_rpm,2)/pow(10,6) - 0.002*vel_rpm +4;
  t_in = -7*pow(vel_rpm,2)/pow(10,6) + 0.0113*vel_rpm + 65.53;
  t_out =-1*pow(vel_rpm,2)/pow(10,5) + 0.0346*vel_rpm + 50.87;
  flow_d = -2*pow(vel_rpm,2)/pow(10,6) + 0.0102*vel_rpm - 5.95;
    } 
